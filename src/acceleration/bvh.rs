...
struct Node {
  aabb: AABB,
  left: Box<dyn Branch + Sync + Send>,
  right: Box<dyn Branch + Sync + Send>,
}
...
pub struct BVH<'a> {
  list: Vec<Object<'a>>,
  root: Box<dyn Branch + Sync + Send>,
}

impl<'a> BVH<'a> {
  pub fn new(objects: Vec<Object<'a>>) -> Self {
    let mut leaf = objects
      .iter()
      .enumerate()
      .map(|(i, v)| Leaf {
        aabb: v.geometry.aabb().clone(),
        index: i,
      })
      .collect::<Vec<_>>();
    let root = Self::build(&mut leaf);
    BVH {
      list: objects,
      root: root,
    }
  ...
  fn build(list: &mut [Leaf]) -> Box<dyn Branch + Sync + Send> {
    // TODO
    let t_aabb = 1.0;
    let t_tri = 2.0;
    // セットアップ
    ...
    // 要素が1つのときは葉
    if n == 1 {
      return Box::new(list[0].clone());
    }
    // 全体のAABB
    let mut aabb = AABB::empty();
    // SAHに基づいた最良の分割軸とインデックスを取得
    let (partition_axis, partition_index, _) = (0..3)
      .map(|axis| {
        // 基準の軸でソート
        list.sort_unstable_by(|a, b| unsafe_cmp(&a.aabb.center[axis], &b.aabb.center[axis]));
        // S1のAABBの表面積
        ...
        (0..n - 1)
          .map(|i| {
            ...
            2.0 * t_aabb + (s1_a[i] * s1_n + s2_a[n - i - 2] * s2_n) * t_tri / s_a
          })
          .enumerate()
          .min_by(|&(_, a), &(_, b)| unsafe_cmp(&a, &b))
          .unwrap()
      })
      .enumerate()
      .min_by(|&(_, (_, a)), &(_, (_, b))| unsafe_cmp(&a, &b))
      .map(|(a, (i, t))| (a, i + 1, t))
      .unwrap();
    // 基準の軸でソート
    list.sort_unstable_by(|a, b| {
      unsafe_cmp(
        &a.aabb.center[partition_axis],
        &b.aabb.center[partition_axis],
      )
    });
    // 再帰的に子要素を生成
    debug_assert!(partition_index != 0 && partition_index != n);
    let left = Self::build(&mut list[0..partition_index]);
    let right = Self::build(&mut list[partition_index..]);
    Box::new(Node {
      aabb: aabb,
      left: left,
      right: right,
...
impl<'a> Interact for BVH<'a> {
  fn interact(&self, ray: Ray) -> Option<Interaction> {
    let mut candidate = Vec::new();
    self.root.may_intersect(&ray, &mut candidate);
    candidate
      .iter()
      .flat_map(|&i| self.list[i].interact(ray))
      .min()
  }
}

impl<'a> Acceleration for BVH<'a> {
